...
class PassengerTripController extends Controller {
  ...
  async getPassengerSearching() {
    const { ctx } = this;
    const data = await ctx.service.passengerTrip.find();
    this.success(data);
  }

  async publishedFindPassenger() {
    const { ctx } = this;
    const params = ctx.request.body;
    // 临时把loginPhone当token
    // params.loginPhone = ctx.cookies.get('loginPhone');
    // console.log('loginPhone', ctx.cookies.get('loginPhone'));
    const data = await ctx.service.passengerTrip.save(params);
    this.success(data);
  }
}
